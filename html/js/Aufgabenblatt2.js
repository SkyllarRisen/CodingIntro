...
function numberGSDS(N) {
  ...
  rest = n2 % 12;
  console.log(
    `N = ${N}: ${groß} Groß, ${schock} Schock, ${dutzend} Dutzend, ${rest} Stück.`
  );
}
...
console.log(
  `${liters} l fuel at ${priceFuel} Euros per liter costs ${priceFuelTotal(
    priceFuel,
    liters
  ).toFixed(2)} Euros`
);
console.log(
  `Without ${salesTax * 100} % sales tax this would be ${priceNetto(
    priceFuelTotal(priceFuel, liters),
    salesTax
  ).toFixed(2)} Euros.`
);

console.log(
  `Volumen Pyramide ${length}, ${height} in Ellen: ` +
    volumePyramid(length, height)
);
console.log(
  `Volumen Pyramide ${length}, ${height} in m^3: ` +
    convertElleToM(volumePyramid(length, height)).toFixed(2)
);
console.log(
  `Volumen Pyramide ${length}, ${height} in Scheffel: ` +
    convertMtoScheffel(convertElleToM(volumePyramid(length, height))).toFixed(2)
);
console.log(
  `Anzahl Steinblöcke: ` + amountBlocks(volumePyramid(length, height))
);
